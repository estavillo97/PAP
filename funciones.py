# -*- coding: utf-8 -*-
"""
Created on Thu Apr 23 20:06:50 2020
...
from ibapi import wrapper,contract
...
version='pochtli 10'
totalcompras=0
print('importanto herramientas utiles')
...
class TestClient(EClient):
    ...
    def place_new_IB_order(self, ibcontract, order, orderid=None):
        ...
        self.placeOrder(
            oid,  # orderId,
            contract_info,#ibcontract,  # contract,
...
'''class TestApp(TestWrapper, TestClient):
    ...
    def bracketSample(self):
        # BRACKET ORDER
        # ! [bracketsubmit]
        bracket = BracketOrder(oid,  Q, tp, sl)
        for o in bracket:
            self.placeOrder(oid, contract_info, o)
...
class coffecup:
    ...
    def drink_coffee(self):
        if self.__temperature>=2021:
            raise runtimeerror('runtime error, this application has requested the runtime to terminate it in an unusual way.')
        else:
            print('descargando informacion de acciones')
...
def BracketOrder(parentOrderId:int, action:str, quantity:float, 
   ...
   if updater>1:   
       parent.faGroup='pochtli'
       parent.faMethod='AvailableEquity'
   else:
       parent.account=cuenta
   parent.tif='GTC'
   ...
   if updater>1:
       takeProfit.faGroup='pochtli'
       takeProfit.FaMethod='AvailableEquity'
   
   else:
       takeProfit.account=cuenta
   takeProfit.tif='GTC'
   ...
   #####stopLoss.adjustedTrailingAmount=stopLossPrice
   #####stopLoss.adjustableTrailingUnit=stopLossPrice
   #####stopLoss.trailStopPrice=stopLossPrice
   ...
   if updater>1:
       stopLoss.faGroup='pochtli'
       stopLoss.faMethod='AvailableEquity'
   else:
       stopLoss.account=cuenta
   stopLoss.tif='GTC'
